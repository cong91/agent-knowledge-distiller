...
export class ScorerService {
  scoreMemory(memory: AgentMemory): ScoredMemory {
    ...
    if (memory.source_agent === 'trader') {
      ...
      else if (text.includes('thua') || text.includes('loss') || text.includes('stop loss')) category = 'trading_loss_lesson';
      else if (tags.includes('technical') || text.includes('market') || text.includes('thị trường')) category = 'market_insight';
      else if (score > 50) category = 'market_insight';
